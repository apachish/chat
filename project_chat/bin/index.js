...
router.post("/register",function(req,res){
    ...
    if(req.body.telephon.length == 10){
        console.log(pub.sismember("Telephon",req.body.telephon));
        if(pub.sismember("Telephon",req.body.telephon)){
            ...
            // pub.SET(req.session.key,information);
            // request(url, function (error, response, body) {
            //     if (!error && response.statusCode == 200) {
            //         console.log('request url: '+url+ key);
            //     }
            // });
        }else{
            res.json({"error" : true , "message" : "Error Number Telephon Exist."});
        }
    }else{
